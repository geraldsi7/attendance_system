...
export default {
    ...
    methods: {
        ...
        submit() {
            if (!this.editContent) {
                this.form.post(route("venue.store"), {
                    onSuccess: () => {
                        this.hideFormModal();
                ...
                this.form.put(route("venue.update", this.form.id), {
                    onSuccess: () => {
                        this.hideFormModal();
        ...
        <Modal :show="formModal" :closeable="true" :modalTitle="this.action + ' item'" @close="hideFormModal"
            ...
            <form @submit.prevent="submit">
                <div class="grid grid-cols-12 gap-5">
                    ...
                    <div class="col-span-full">
                        <InputLabel for="longitude" value="Longitude" :required="true" />
                        <TextInput id="longitude" type="text" class="w-full" v-model="form.longitude" :placeholder="'Longitude'"
                            autocomplete="longitude" :class="{ 'border-red-600': form.errors.longitude }" />
                        <InputError :message="form.errors.longitude" class="mt-2" />
                    </div>

                    <div class="col-span-full">
                        <InputLabel for="latitude" value="Latitude" :required="true" />
                        <TextInput id="latitude" type="text" class="w-full" v-model="form.latitude" :placeholder="'Latitude'"
                            autocomplete="latitude" :class="{ 'border-red-600': form.errors.latitude }" />
